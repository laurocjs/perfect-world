...
module.exports = {
  ...
  insertUserPreferences: function (user, character, sky, ground, textbox, textbox_title, audio_path) {
    pool.query('insert into user_preferences(user_fk,character,sky,ground,textbox,textbox_title,audio_path) values ($1,$2,$3,$4,$5,$6,$7)', [user, character, sky, ground, textbox, textbox_title, audio_path], (err, res) => {
      if (err) {
        console.log(err);
        res.status(400).send(err);
      }
